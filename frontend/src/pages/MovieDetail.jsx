...
export default function MovieDetail() {
  ...
  const salvarNota = async () => {
    ...
    try {
      ...
      await load(); // recarrega dados (média, contagem, etc.)
    } catch (e) {
  ...
  const media = normAvg(m);
  const qtd = normCount(m);
  ...
  return (
    <div className="grid md:grid-cols-3 gap-6">
      ...
      <div className="md:col-span-2 card">
        ...
        <p className="text-zinc-600">{genre} {director && <>• {director}</>}</p>

        <div className="mt-3 text-sm text-zinc-700">
          Média: {media ?? "-"} ({qtd} avaliações)
        </div>
